...
You've given the root node of a Binary Tree.  Write a function that returns true 
...
Hint 1
To solve this problem, you'll have to determine if every subtree in the Binary 
Tree is balanced.  Which subtrees do you know will always be balanced?

Hint 2
To determine if a subtree is balanced, you need to know the height of it left 
and right subtrees.  The only exception to this is if a subtree has no left and 
right subtrees (i.e., it's just a leaf node); in that case, the subtree must be 
balanced.

Hint 3
Recursively calculate the left and right subtree heights from each node.
Once you know the heights of a particular node's left and right subtrees, you 
can determine if the subtree rooted at that node is balanced.  If a subtree 
ever isn't balanced, you can immediately conclude that the entire tree isn't 
...
subtrees aren't more than 1 apart, then the entire tree is balanced.
