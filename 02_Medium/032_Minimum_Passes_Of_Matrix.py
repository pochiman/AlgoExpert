...
that is to the left, to the right, above, or below the current element in the 
...
second queue you created (the one with the positions of negatives that were 
changed to positives). Repeat this process until no values are converted during 
a pass.
...
O(w * h) time | O(w * h) space - where w is the width of the matrix and h 
is the height
